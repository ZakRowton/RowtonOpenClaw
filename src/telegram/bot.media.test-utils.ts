...
import * as ssrf from "../infra/net/ssrf.ts";
import { onSpy, sendChatActionSpy } from "./bot.media.e2e-harness.ts";
...
let createTelegramBotRef: typeof import("./bot.ts").createTelegramBot;
...
beforeAll(async () => {
  ({ createTelegramBot: createTelegramBotRef } = await import("./bot.ts"));
  const replyModule = await import("../auto-reply/reply.ts");
