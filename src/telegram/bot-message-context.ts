...
import type { StickerMetadata, TelegramContext } from "./bot/types.ts";
import { enforceTelegramDmAccess } from "./dm-access.ts";
import { evaluateTelegramGroupBaseAccess } from "./group-access.ts";
import { resolveTelegramGroupPromptSettings } from "./group-config-helpers.ts";
import {
  buildTelegramStatusReactionVariants,
  resolveTelegramAllowedEmojiReactions,
  resolveTelegramReactionVariant,
  resolveTelegramStatusReactionEmojis,
} from "./status-reaction-variants.ts";
...
export const buildTelegramMessageContext = async ({
  ...
  if (needsPreflightTranscription) {
    try {
      const { transcribeFirstAudio } = await import("../media-understanding/audio-preflight.ts");
