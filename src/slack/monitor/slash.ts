...
import { chunkItems } from "../../utils/chunk-items.ts";
...
} from "./external-arg-menu-store.ts";
import { escapeSlackMrkdwn } from "./mrkdwn.ts";
import { isSlackChannelAllowedByPolicy } from "./policy.ts";
import { resolveSlackRoomContextHints } from "./room-context.ts";
...
type CommandsRegistry = typeof import("../../auto-reply/commands-registry.ts");
let commandsRegistry: CommandsRegistry | undefined;
async function getCommandsRegistry(): Promise<CommandsRegistry> {
  if (!commandsRegistry) {
    commandsRegistry = await import("../../auto-reply/commands-registry.ts");
  }
...
export async function registerSlackMonitorSlashCommands(params: {
  ...
  const handleSlashCommand = async (p: {
    ...
    try {
      ...
      const [{ resolveAgentRoute }, { finalizeInboundContext }, { dispatchReplyWithDispatcher }] =
        await Promise.all([
          import("../../routing/resolve-route.ts"),
          import("../../auto-reply/reply/inbound-context.ts"),
          import("../../auto-reply/reply/provider-dispatcher.ts"),
        ]);
      const [
        ...
        import("../../channels/conversation-label.ts"),
        import("../../channels/reply-prefix.ts"),
        import("../../config/sessions.ts"),
      ]);
      ...
      const deliverSlashPayloads = async (replies: ReplyPayload[]) => {
        const [{ deliverSlackSlashReplies }, { resolveChunkMode }, { resolveMarkdownTableMode }] =
          await Promise.all([
            import("./replies.ts"),
            import("../../auto-reply/chunk.ts"),
            import("../../config/markdown-tables.ts"),
          ]);
  ...
  if (nativeEnabled) {
    ...
    const skillCommands = nativeSkillsEnabled
      ? (await import("../../auto-reply/skill-commands.ts")).listSkillCommandsForAgents({ cfg })
      : [];
