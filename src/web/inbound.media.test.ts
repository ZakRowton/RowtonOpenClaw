...
vi.mock("../media/store.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../media/store.ts")>();
  return {
    ...actual,
...
import { monitorWebInbox, resetWebInboundDedupe } from "./inbound.ts";
let createWaSocket: typeof import("./session.ts").createWaSocket;
...
describe("web inbound media saves with extension", () => {
  ...
  beforeAll(async () => {
    ({ createWaSocket } = await import("./session.ts"));
