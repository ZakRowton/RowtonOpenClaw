...
import { escapeRegExp, formatEnvelopeTimestamp } from "../../test/helpers/envelope-timestamp.ts";
...
import {
  ...
  makeSessionStore,
  setLoadConfigMock,
} from "./auto-reply.test-harness.ts";
import type { WebInboundMessage } from "./inbound.ts";
...
describe("web auto-reply", () => {
  ...
  let monitorWebChannel: typeof import("./auto-reply.ts").monitorWebChannel;
  beforeAll(async () => {
    ({ monitorWebChannel } = await import("./auto-reply.ts"));
  });
  ...
  it("forces reconnect when watchdog closes without onClose", async () => {
    ...
    try {
      ...
      let capturedOnMessage:
        | ((msg: import("./inbound.ts").WebInboundMessage) => Promise<void>)
        | undefined;
      const listenerFactory = vi.fn(
        async (opts: {
          onMessage: (msg: import("./inbound.ts").WebInboundMessage) => Promise<void>;
        }) => {
