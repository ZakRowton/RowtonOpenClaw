...
import { DEFAULT_EXEC_APPROVAL_TIMEOUT_MS } from "../../infra/exec-approvals.ts";
...
describe("nodes run: approval transport timeout (#12098)", () => {
  let callGatewayCli: typeof import("./rpc.ts").callGatewayCli;

  beforeAll(async () => {
    ({ callGatewayCli } = await import("./rpc.ts"));
  });
