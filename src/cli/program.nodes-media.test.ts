...
import { IOS_NODE, createIosNodeListResponse } from "./program.nodes-test-helpers.ts";
import { callGateway, installBaseProgramMocks, runtime } from "./program.test-mocks.ts";
...
describe("cli program (nodes media)", () => {
  ...
  beforeAll(async () => {
    ({ registerNodesCli } = await import("./nodes-cli.ts"));
    program = new Command();
