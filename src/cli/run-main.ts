...
import { getCommandPath, getPrimaryCommand, hasHelpOrVersion } from "./argv.ts";
...
export async function runCli(argv: string[] = process.argv) {
  ...
  if (primary) {
    const { getProgramContext } = await import("./program/program-context.ts");
    const ctx = getProgramContext(program);
    if (ctx) {
      const { registerCoreCliByName } = await import("./program/command-registry.ts");
