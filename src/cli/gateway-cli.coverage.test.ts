...
import { withEnvOverride } from "../config/test-helpers.ts";
import { createCliRuntimeCapture } from "./test-runtime-capture.ts";

type DiscoveredBeacon = Awaited<
  ReturnType<typeof import("../infra/bonjour-discovery.ts").discoverGatewayBeacons>
>[number];
...
const { registerGatewayCli } = await import("./gateway-cli.ts");
...
describe("gateway-cli coverage", () => {
  ...
  it("prints stop hints on GatewayLockError when service is loaded", async () => {
    ...
    const { GatewayLockError } = await import("../infra/gateway-lock.ts");
