...
import type { OpenClawConfig, ConfigFileSnapshot } from "../config/types.openclaw.ts";
...
vi.mock("../infra/update-check.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../infra/update-check.ts")>();
  return {
    ...actual,
...
vi.mock("./update-cli/shared.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./update-cli/shared.ts")>();
  return {
    ...actual,
    readPackageName,
    readPackageVersion,
...
const { runGatewayUpdate } = await import("../infra/update-runner.ts");
const { resolveOpenClawPackageRoot } = await import("../infra/openclaw-root.ts");
const { readConfigFileSnapshot, writeConfigFile } = await import("../config/config.ts");
const { checkUpdateStatus, fetchNpmTagVersion, resolveNpmChannelTag } =
  await import("../infra/update-check.ts");
const { runCommandWithTimeout } = await import("../process/exec.ts");
const { runDaemonRestart, runDaemonInstall } = await import("./daemon-cli.ts");
const { doctorCommand } = await import("../commands/doctor.ts");
const { defaultRuntime } = await import("../runtime.ts");
const { updateCommand, registerUpdateCli, updateStatusCommand, updateWizardCommand } =
  await import("./update-cli.ts");
