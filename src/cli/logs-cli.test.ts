...
import { runRegisteredCli } from "../test-utils/command-runner.ts";
import { formatLogTimestamp } from "./logs-cli.ts";

const callGatewayFromCli = vi.fn();

vi.mock("./gateway-rpc.js", async () => {
  const actual = await vi.importActual<typeof import("./gateway-rpc.ts")>("./gateway-rpc.js");
  return {
    ...actual,
...
let registerLogsCli: typeof import("./logs-cli.ts").registerLogsCli;

beforeAll(async () => {
  ({ registerLogsCli } = await import("./logs-cli.ts"));
});
