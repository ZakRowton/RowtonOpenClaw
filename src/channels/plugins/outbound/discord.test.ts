...
import { normalizeDiscordOutboundTarget } from "../normalize/discord.ts";
...
vi.mock("../../../discord/send.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../../discord/send.ts")>();
  return {
    ...actual,
...
vi.mock("../../../discord/monitor/thread-bindings.js", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../../../discord/monitor/thread-bindings.ts")>();
  return {
    ...actual,
...
const { discordOutbound } = await import("./discord.ts");
