...
import { withEnvAsync } from "../test-utils/env.ts";
...
import { ConnectErrorDetailCodes } from "./protocol/connect-error-details.ts";
...
async function createSignedDevice(params: {
  ...
  const { loadOrCreateDeviceIdentity, publicKeyRawBase64UrlFromPem, signDevicePayload } =
    await import("../infra/device-identity.ts");
...
async function approvePendingPairingIfNeeded() {
  const { approveDevicePairing, listDevicePairing } = await import("../infra/device-pairing.ts");
...
async function startRateLimitedTokenServerWithPairedDeviceToken() {
  const { loadOrCreateDeviceIdentity } = await import("../infra/device-identity.ts");
  const { getPairedDevice } = await import("../infra/device-pairing.ts");
...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("connect (req) handshake returns hello-ok payload", async () => {
      const { CONFIG_PATH, STATE_DIR } = await import("../config/config.ts");
      const ws = await openWs(port);
  ...
  test("does not bypass pairing for control ui device identity when insecure auth is enabled", async () => {
    ...
    const { writeConfigFile } = await import("../config/config.ts");
  ...
  test("skips pairing for operator scope upgrades when shared token auth is valid", async () => {
    ...
    const { buildDeviceAuthPayload } = await import("./device-auth.ts");
    const { loadOrCreateDeviceIdentity, publicKeyRawBase64UrlFromPem, signDevicePayload } =
      await import("../infra/device-identity.ts");
    const { getPairedDevice, listDevicePairing } = await import("../infra/device-pairing.ts");
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
  ...
  test("auto-approves loopback scope upgrades for control ui clients", async () => {
    ...
    const { approveDevicePairing, getPairedDevice, listDevicePairing, requestDevicePairing } =
      await import("../infra/device-pairing.ts");
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
  ...
  test("allows operator.read connect when device is paired with operator.admin", async () => {
    ...
    const { loadOrCreateDeviceIdentity, publicKeyRawBase64UrlFromPem, signDevicePayload } =
      await import("../infra/device-identity.ts");
    const { listDevicePairing } = await import("../infra/device-pairing.ts");
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "openclaw-device-scope-"));
    const identity = loadOrCreateDeviceIdentity(join(identityDir, "device.json"));
    const client = {
  ...
  test("allows operator shared auth with legacy paired metadata", async () => {
    ...
    const { loadOrCreateDeviceIdentity, publicKeyRawBase64UrlFromPem, signDevicePayload } =
      await import("../infra/device-identity.ts");
    const { resolvePairingPaths, readJsonFile } = await import("../infra/pairing-files.ts");
    const { writeJsonAtomic } = await import("../infra/json-files.ts");
    const { approveDevicePairing, getPairedDevice, listDevicePairing, requestDevicePairing } =
      await import("../infra/device-pairing.ts");
  ...
  test("allows shared-auth scope escalation even when paired metadata is legacy-shaped", async () => {
    ...
    const { tmpdir } = await import("node:os");
    const { join } = await import("node:path");
    const { readJsonFile, resolvePairingPaths } = await import("../infra/pairing-files.ts");
    const { writeJsonAtomic } = await import("../infra/json-files.ts");
    const { buildDeviceAuthPayload } = await import("./device-auth.ts");
    const { loadOrCreateDeviceIdentity, publicKeyRawBase64UrlFromPem, signDevicePayload } =
      await import("../infra/device-identity.ts");
    ...
    const { GATEWAY_CLIENT_MODES, GATEWAY_CLIENT_NAMES } =
      await import("../utils/message-channel.ts");
  ...
  test("rejects revoked device token", async () => {
    const { revokeDeviceToken } = await import("../infra/device-pairing.ts");
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
