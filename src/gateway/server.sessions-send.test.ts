...
const { createOpenClawTools } = await import("../agents/openclaw-tools.ts");
...
beforeAll(async () => {
  ...
  const { approveDevicePairing, requestDevicePairing } = await import("../infra/device-pairing.ts");
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      const { callGateway } = await import("./call.ts");
      await callGateway({
