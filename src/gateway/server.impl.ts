...
import { getActiveEmbeddedRunCount } from "../agents/pi-embedded-runner/runs.ts";
...
import { isRestartEnabled } from "../config/commands.ts";
import {
  CONFIG_PATH,
  isNixMode,
  loadConfig,
...
import { getGlobalHookRunner, runGlobalGatewayStopSafely } from "../plugins/hook-runner-global.ts";
...
import { ensureGatewayStartupAuth } from "./startup-auth.ts";
...
export async function startGatewayServer(
  ...
  if (!minimalTestGateway) {
    void (async () => {
      const { recoverPendingDeliveries } = await import("../infra/outbound/delivery-queue.ts");
