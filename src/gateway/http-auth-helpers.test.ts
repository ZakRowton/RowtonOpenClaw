...
import { authorizeGatewayBearerRequestOrReply } from "./http-auth-helpers.ts";
...
const { authorizeHttpGatewayConnect } = await import("./auth.ts");
const { sendGatewayAuthFailure } = await import("./http-common.ts");
const { getBearerToken } = await import("./http-utils.ts");
