...
import { GATEWAY_CLIENT_IDS, GATEWAY_CLIENT_MODES } from "./protocol/client-info.ts";
import { startGatewayServerHarness, type GatewayServerHarness } from "./server.e2e-ws-harness.ts";
import { createToolSummaryPreviewTranscriptLines } from "./session-preview.test-helpers.ts";
import {
  connectOk,
  embeddedRunMock,
...
vi.mock("../auto-reply/reply/queue.js", async () => {
  const actual = await vi.importActual<typeof import("../auto-reply/reply/queue.ts")>(
...
vi.mock("../auto-reply/reply/abort.js", async () => {
  const actual = await vi.importActual<typeof import("../auto-reply/reply/abort.ts")>(
...
vi.mock("../hooks/internal-hooks.js", async () => {
  const actual = await vi.importActual<typeof import("../hooks/internal-hooks.ts")>(
...
vi.mock("../plugins/hook-runner-global.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../plugins/hook-runner-global.ts")>();
  return {
    ...actual,
...
vi.mock("../discord/monitor/thread-bindings.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../discord/monitor/thread-bindings.ts")>();
  return {
    ...actual,
