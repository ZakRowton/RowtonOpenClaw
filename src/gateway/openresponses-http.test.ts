...
import { HISTORY_CONTEXT_MARKER } from "../auto-reply/reply/history.ts";
import { CURRENT_MESSAGE_MARKER } from "../auto-reply/reply/mentions.ts";
import { emitAgentEvent } from "../infra/agent-events.ts";
import { buildAssistantDeltaResult } from "./test-helpers.agent-results.ts";
import { agentCommand, getFreePort, installGatewayTestHooks } from "./test-helpers.ts";
...
async function startServer(port: number, opts?: { openResponsesEnabled?: boolean }) {
  const { startGatewayServer } = await import("./server.ts");
  const serverOpts = {
    host: "127.0.0.1",
