...
import { GATEWAY_CLIENT_CAPS } from "../protocol/client-info.ts";
...
vi.mock("../session-utils.js", async (importOriginal) => {
  const original = await importOriginal<typeof import("../session-utils.ts")>();
  return {
    ...original,
    loadSessionEntry: () => ({
      cfg: {},
...
const { chatHandlers } = await import("./chat.ts");
