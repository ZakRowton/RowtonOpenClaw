...
import { validateExecApprovalRequestParams } from "../protocol/index.ts";
...
import { sanitizeChatSendMessageInput } from "./chat.ts";
import { createExecApprovalHandlers } from "./exec-approval.ts";
...
describe("gateway healthHandlers.status scope handling", () => {
  let statusModule: typeof import("../../commands/status.ts");
  let healthHandlers: typeof import("./health.ts").healthHandlers;

  beforeAll(async () => {
    statusModule = await import("../../commands/status.ts");
    ({ healthHandlers } = await import("./health.ts"));
  });
