...
import { createChannelTestPluginBase } from "../test-utils/channel-plugins.ts";
import { setRegistry } from "./server.agent.gateway-server-agent.mocks.ts";
import { createRegistry } from "./server.e2e-registry-helpers.ts";
import {
  connectOk,
...
let readConfigFileSnapshot: typeof import("../config/config.ts").readConfigFileSnapshot;
let writeConfigFile: typeof import("../config/config.ts").writeConfigFile;
...
beforeAll(async () => {
  ({ readConfigFileSnapshot, writeConfigFile } = await import("../config/config.ts"));
