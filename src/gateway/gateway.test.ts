...
} from "./test-helpers.e2e.ts";
import { installOpenAiResponsesMock } from "./test-helpers.openai-mock.ts";
import { buildOpenAiResponsesProviderConfig } from "./test-openai-responses-model.ts";

let writeConfigFile: typeof import("../config/config.ts").writeConfigFile;
let resolveConfigPath: typeof import("../config/config.ts").resolveConfigPath;
const GATEWAY_E2E_TIMEOUT_MS = 30_000;

describe("gateway e2e", () => {
  beforeAll(async () => {
    ({ writeConfigFile, resolveConfigPath } = await import("../config/config.ts"));
  });
