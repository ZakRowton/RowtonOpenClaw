...
import { extractAssistantText, sanitizeTextContent } from "./sessions-helpers.ts";
...
import { createSessionsListTool } from "./sessions-list-tool.ts";
import { createSessionsSendTool } from "./sessions-send-tool.ts";

let resolveAnnounceTarget: (typeof import("./sessions-announce-target.ts"))["resolveAnnounceTarget"];
let setActivePluginRegistry: (typeof import("../../plugins/runtime.ts"))["setActivePluginRegistry"];
...
beforeAll(async () => {
  ({ resolveAnnounceTarget } = await import("./sessions-announce-target.ts"));
  ({ setActivePluginRegistry } = await import("../../plugins/runtime.ts"));
});
