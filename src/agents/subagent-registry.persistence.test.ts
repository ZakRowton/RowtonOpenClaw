...
import "./subagent-registry.mocks.shared.ts";
...
import { loadSubagentRegistryFromDisk } from "./subagent-registry.store.ts";
...
describe("subagent registry persistence", () => {
  ...
  it("uses isolated temp state when OPENCLAW_STATE_DIR is unset in tests", async () => {
    ...
    vi.resetModules();
    const { resolveSubagentRegistryPath } = await import("./subagent-registry.store.ts");
