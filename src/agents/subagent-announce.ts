...
import { createBoundDeliveryRouter } from "../infra/outbound/bound-delivery-router.ts";
...
import { normalizeAccountId, normalizeMainKey } from "../routing/session-key.ts";
...
import { isDeliverableMessageChannel, isInternalMessageChannel } from "../utils/message-channel.ts";
import {
...
import type { SpawnSubagentMode } from "./subagent-spawn.ts";
import { readLatestAssistantReply } from "./tools/agent-step.ts";
import { sanitizeTextContent, extractAssistantText } from "./tools/sessions-helpers.ts";
import { isAnnounceSkip } from "./tools/sessions-send-helpers.ts";
  ...
  try {
    ...
    if (
      ...
      if (!forceBoundSessionDirectDelivery) {
        ...
        try {
          const { countActiveDescendantRuns } = await import("./subagent-registry.ts");
  ...
  try {
    ...
    if (requesterIsSubagent) {
      const { isSubagentSessionRunActive, resolveRequesterForChildSession } =
        await import("./subagent-registry.ts");
