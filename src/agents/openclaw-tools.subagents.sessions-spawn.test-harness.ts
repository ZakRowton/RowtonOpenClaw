...
type SessionsSpawnTestConfig = ReturnType<(typeof import("../config/config.ts"))["loadConfig"]>;
type CreateSessionsSpawnTool =
  (typeof import("./tools/sessions-spawn-tool.ts"))["createSessionsSpawnTool"];
...
export async function getSessionsSpawnTool(opts: CreateOpenClawToolsOpts) {
  ...
  const { createSessionsSpawnTool } = await import("./tools/sessions-spawn-tool.ts");
