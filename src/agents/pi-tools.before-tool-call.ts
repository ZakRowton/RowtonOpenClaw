...
async function recordLoopOutcome(args: {
  ...
  try {
    const { getDiagnosticSessionState } = await import("../logging/diagnostic-session-state.ts");
    const { recordToolCallOutcome } = await import("./tool-loop-detection.ts");
...
export async function runBeforeToolCallHook(args: {
  ...
  if (args.ctx?.sessionKey) {
    const { getDiagnosticSessionState } = await import("../logging/diagnostic-session-state.ts");
    const { logToolLoopAction } = await import("../logging/diagnostic.ts");
    const { detectToolCallLoop, recordToolCall } = await import("./tool-loop-detection.ts");
