...
import type { EmbeddedRunAttemptResult } from "./pi-embedded-runner/run/types.ts";
...
vi.mock("./models-config.js", async (importOriginal) => {
  const mod = await importOriginal<typeof import("./models-config.ts")>();
  return {
    ...mod,
...
let runEmbeddedPiAgent: typeof import("./pi-embedded-runner/run.ts").runEmbeddedPiAgent;

beforeAll(async () => {
  ({ runEmbeddedPiAgent } = await import("./pi-embedded-runner/run.ts"));
});
