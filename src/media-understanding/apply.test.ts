...
import { clearMediaUnderstandingBinaryCacheForTests } from "./runner.ts";
...
let applyMediaUnderstanding: typeof import("./apply.ts").applyMediaUnderstanding;
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const execModule = await import("../process/exec.ts");
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    ...
    ({ applyMediaUnderstanding } = await import("./apply.ts"));
  });
