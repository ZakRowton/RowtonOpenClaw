...
import { runCapability } from "./runner.ts";
import { withMediaFixture } from "./runner.test-utils.ts";

async function withVideoFixture(
  filePrefix: string,
  run: (params: {
    ctx: { MediaPath: string; MediaType: string };
    media: ReturnType<typeof import("./runner.ts").normalizeMediaAttachments>;
    cache: ReturnType<typeof import("./runner.ts").createMediaAttachmentCache>;
  }) => Promise<void>,
) {
