...
import { loadSessionStore, saveSessionStore, type SessionEntry } from "../../config/sessions.ts";
...
vi.mock(
  ...
  async () => await import("../../test-utils/model-fallback.mock.ts"),
);
...
vi.mock("./route-reply.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./route-reply.ts")>();
  return {
    ...actual,
