...
import * as sessions from "../../config/sessions.ts";
import type { TypingMode } from "../../config/types.ts";
import { withStateDirEnv } from "../../test-helpers/state-dir-env.ts";
...
import { createMockTypingController } from "./test-helpers.ts";
...
let modelFallbackModule: typeof import("../../agents/model-fallback.ts");
let onAgentEvent: typeof import("../../infra/agent-events.ts").onAgentEvent;

let runReplyAgentPromise:
  | Promise<(typeof import("./agent-runner.ts"))["runReplyAgent"]>
  | undefined;

async function getRunReplyAgent() {
  if (!runReplyAgentPromise) {
    runReplyAgentPromise = import("./agent-runner.ts").then((m) => m.runReplyAgent);
  }
...
beforeAll(async () => {
  ...
  modelFallbackModule = await import("../../agents/model-fallback.ts");
  ({ onAgentEvent } = await import("../../infra/agent-events.ts"));
