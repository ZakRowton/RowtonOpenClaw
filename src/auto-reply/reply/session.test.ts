...
import { buildModelAliasIndex } from "../../agents/model-selection.ts";
import type { OpenClawConfig } from "../../config/config.ts";
import type { SessionEntry } from "../../config/sessions.ts";
import { saveSessionStore } from "../../config/sessions.ts";
import { formatZonedTimestamp } from "../../infra/format-time/format-datetime.ts";
import { enqueueSystemEvent, resetSystemEventsForTest } from "../../infra/system-events.ts";
import { applyResetModelOverride } from "./session-reset-model.ts";
import { prependSystemEvents } from "./session-updates.ts";
...
describe("initSessionState preserves behavior overrides across /new and /reset", () => {
  ...
  it("archives the old session store entry on /new", async () => {
    ...
    const sessionUtils = await import("../../gateway/session-utils.fs.ts");
