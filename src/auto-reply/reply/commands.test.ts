...
import { compactEmbeddedPiSession } from "../../agents/pi-embedded.ts";
import {
...
import * as internalHooks from "../../hooks/internal-hooks.ts";
import { clearPluginCommands, registerPluginCommand } from "../../plugins/commands.ts";
import { typedCases } from "../../test-utils/typed-cases.ts";
import type { MsgContext } from "../templating.ts";
import { resetBashChatCommandForTests } from "./bash-command.ts";
import { handleCompactCommand } from "./commands-compact.ts";
import { buildCommandsPaginationKeyboard } from "./commands-info.ts";
import { extractMessageText } from "./commands-subagents.ts";
import { buildCommandTestParams } from "./commands.test-harness.ts";
...
vi.mock("../../config/config.js", async () => {
  const actual =
    await vi.importActual<typeof import("../../config/config.ts")>("../../config/config.js");
  return {
    ...actual,
...
vi.mock("../../pairing/pairing-store.js", async () => {
  const actual = await vi.importActual<typeof import("../../pairing/pairing-store.ts")>(
...
vi.mock("../../channels/plugins/pairing.js", async () => {
  const actual = await vi.importActual<typeof import("../../channels/plugins/pairing.ts")>(
...
import { buildCommandContext, handleCommands } from "./commands.ts";
