...
import { createThrowingRuntime, readJsonFile } from "./onboard-non-interactive.test-helpers.ts";
...
const { resolveConfigPath: resolveStateConfigPath } = await import("../config/paths.ts");
const { resolveConfigPath } = await import("../config/config.ts");
const { callGateway } = await import("../gateway/call.ts");
