...
import {
  ...
  serviceRestart,
  writeConfigFile,
} from "./doctor.e2e-harness.ts";

let doctorCommand: typeof import("./doctor.ts").doctorCommand;
let healthCommand: typeof import("./health.ts").healthCommand;

describe("doctor command", () => {
  beforeAll(async () => {
    ({ doctorCommand } = await import("./doctor.ts"));
    ({ healthCommand } = await import("./health.ts"));
  });
