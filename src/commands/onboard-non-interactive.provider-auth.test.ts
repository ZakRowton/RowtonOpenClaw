...
import { makeTempWorkspace } from "../test-helpers/workspace.ts";
import { withEnvAsync } from "../test-utils/env.ts";
import { MINIMAX_API_BASE_URL, MINIMAX_CN_API_BASE_URL } from "./onboard-auth.ts";
import {
  createThrowingRuntime,
  readJsonFile,
  type NonInteractiveRuntime,
} from "./onboard-non-interactive.test-helpers.ts";
import { OPENAI_DEFAULT_MODEL } from "./openai-model-default.ts";
...
vi.mock("./onboard-helpers.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./onboard-helpers.ts")>();
  return {
    ...actual,
...
const { runNonInteractiveOnboarding } = await import("./onboard-non-interactive.ts");
...
let ensureAuthProfileStore: typeof import("../agents/auth-profiles.ts").ensureAuthProfileStore;
let upsertAuthProfile: typeof import("../agents/auth-profiles.ts").upsertAuthProfile;
...
describe("onboard (non-interactive): provider auth", () => {
  beforeAll(async () => {
    ({ ensureAuthProfileStore, upsertAuthProfile } = await import("../agents/auth-profiles.ts"));
  });
