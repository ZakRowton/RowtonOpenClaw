...
vi.mock("./list.registry.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./list.registry.ts")>();
  return {
    ...actual,
...
vi.mock("../../agents/model-forward-compat.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../agents/model-forward-compat.ts")>();
  return {
    ...actual,
...
import { modelsListCommand } from "./list.list-command.ts";
