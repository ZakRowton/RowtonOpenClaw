...
vi.mock("../infra/ssh-tunnel.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../infra/ssh-tunnel.ts")>();
  return {
    ...actual,
    startSshPortForward,
  };
...
describe("gateway-status command", () => {
  it("prints human output by default", async () => {
    ...
    const { gatewayStatusCommand } = await import("./gateway-status.ts");
    await gatewayStatusCommand(
      { timeout: "1000" },
      runtime as unknown as import("../runtime.ts").RuntimeEnv,
    );
