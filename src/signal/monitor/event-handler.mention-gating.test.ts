...
import { buildDispatchInboundCaptureMock } from "../../../test/helpers/dispatch-inbound-capture.ts";
import type { MsgContext } from "../../auto-reply/templating.ts";
import type { OpenClawConfig } from "../../config/types.ts";
import {
  createBaseSignalEventHandlerDeps,
  createSignalReceiveEvent,
} from "./event-handler.test-harness.ts";
...
vi.mock("../../auto-reply/dispatch.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../auto-reply/dispatch.ts")>();
  return buildDispatchInboundCaptureMock(actual, (ctx) => {
    capturedCtx = ctx as SignalMsgContext;
  });
});

import { createSignalEventHandler } from "./event-handler.ts";
import { renderSignalMentions } from "./mentions.ts";
