...
import { isPathWithinBase } from "../../test/helpers/paths.ts";
import { createTempHomeEnv, type TempHomeEnv } from "../test-utils/temp-home.ts";
...
describe("media store", () => {
  ...
  async function withTempStore<T>(
    fn: (store: typeof import("./store.ts"), home: string) => Promise<T>,
  ): Promise<T> {
  ...
  it("prefers header mime extension when sniffed mime lacks mapping", async () => {
    await withTempStore(async (_store, home) => {
      ...
      vi.doMock("./mime.js", async () => {
        const actual = await vi.importActual<typeof import("./mime.ts")>("./mime.js");
        return {
          ...actual,
      ...
      try {
        const storeWithMock = await import("./store.ts");
