...
import { packNpmSpecToArchive, withTempDir } from "./install-source-utils.ts";
import type { NpmIntegrityDriftPayload } from "./npm-integrity.ts";
import {
  finalizeNpmSpecArchiveInstall,
  installFromNpmSpecArchive,
  installFromNpmSpecArchiveWithInstaller,
} from "./npm-pack-install.ts";

vi.mock("./install-source-utils.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./install-source-utils.ts")>();
  return {
    ...actual,
