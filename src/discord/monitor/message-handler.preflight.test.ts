...
function createThreadBinding(
  overrides?: Partial<import("./thread-bindings.ts").ThreadBindingRecord>,
) {
  return {
    accountId: "default",
    channelId: "parent-1",
    ...
    webhookToken: "tok-1",
    ...overrides,
  } satisfies import("./thread-bindings.ts").ThreadBindingRecord;
}
...
describe("preflightDiscordMessage", () => {
  it("bypasses mention gating in bound threads for allowed bot senders", async () => {
    ...
    const result = await preflightDiscordMessage({
      ...
      } as import("../../config/config.ts").OpenClawConfig,
      discordConfig: {
        allowBots: true,
      } as NonNullable<import("../../config/config.ts").OpenClawConfig["channels"]>["discord"],
      accountId: "default",
      token: "token",
      runtime: {} as import("../../runtime.ts").RuntimeEnv,
      ...
      } as import("./thread-bindings.ts").ThreadBindingManager,
      data: {
        channel_id: threadId,
        guild_id: "guild-1",
        guild: {
        ...
        message,
      } as unknown as import("./listeners.ts").DiscordMessageEvent,
      client,
    });
