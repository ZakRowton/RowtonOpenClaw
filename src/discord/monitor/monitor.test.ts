...
import type { DiscordAccountConfig } from "../../config/types.discord.ts";
...
import type { DiscordComponentEntry, DiscordModalEntry } from "../components.ts";
import {
...
import { clearPresences, getPresence, presenceCacheSize, setPresence } from "./presence-cache.ts";
...
vi.mock("../../infra/system-events.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../infra/system-events.ts")>();
  return {
    ...actual,
...
vi.mock("../../config/sessions.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../config/sessions.ts")>();
  return {
    ...actual,
  ...
  function createAutoThreadParams(client: Client) {
    return {
      ...
      message: {
        id: "m1",
        channelId: "parent",
      } as unknown as import("./listeners.ts").DiscordMessageEvent["message"],
      isGuildMessage: true,
      channelConfig: {
        autoThread: true,
