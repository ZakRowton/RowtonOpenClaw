...
import { DiscordMessageListener, DiscordReactionListener } from "./monitor/listeners.ts";
...
describe("DiscordMessageListener", () => {
  ...
  it("awaits the handler before returning", async () => {
    ...
    const handlePromise = listener.handle(
      {} as unknown as import("./monitor/listeners.ts").DiscordMessageEvent,
  ...
  it("logs handler failures", async () => {
    const logger = {
      warn: vi.fn(),
      error: vi.fn(),
    } as unknown as ReturnType<typeof import("../logging/subsystem.ts").createSubsystemLogger>;
    const handler = vi.fn(async () => {
...
function makeReactionListenerParams(overrides?: {
  ...
  return {
    cfg: {} as ReturnType<typeof import("../config/config.ts").loadConfig>,
    accountId: "acc-1",
