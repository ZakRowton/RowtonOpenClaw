...
import { resolveAgentModelFallbackValues, resolveAgentModelPrimaryValue } from "./model-input.ts";

const { loadConfig, migrateLegacyConfig, readConfigFileSnapshot, validateConfigObject } =
  await vi.importActual<typeof import("./config.ts")>("./config.js");
import { withTempHome } from "./test-helpers.ts";
