...
import type {
  ...
  ChannelPlugin,
} from "../src/channels/plugins/types.ts";
import type { OpenClawConfig } from "../src/config/config.ts";
import type { OutboundSendDeps } from "../src/infra/outbound/deliver.ts";
import { withIsolatedTestHome } from "./test-env.ts";
...
const [{ installProcessWarningFilter }, { setActivePluginRegistry }, { createTestRegistry }] =
  await Promise.all([
    import("../src/infra/warning-filter.ts"),
    import("../src/plugins/runtime.ts"),
    import("../src/test-utils/channel-plugins.ts"),
  ]);
