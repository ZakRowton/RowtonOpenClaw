...
import { listMSTeamsDirectoryGroupsLive, listMSTeamsDirectoryPeersLive } from "./directory-live.ts";
import { msteamsOnboardingAdapter } from "./onboarding.ts";
import { msteamsOutbound } from "./outbound.ts";
import { resolveMSTeamsGroupToolPolicy } from "./policy.ts";
import { probeMSTeams } from "./probe.ts";
import {
...
import { sendAdaptiveCardMSTeams, sendMessageMSTeams } from "./send.ts";
...
export const msteamsPlugin: ChannelPlugin<ResolvedMSTeamsAccount> = {
  ...
  gateway: {
    startAccount: async (ctx) => {
      const { monitorMSTeamsProvider } = await import("./index.ts");
