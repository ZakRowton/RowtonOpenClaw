...
import { ZalouserConfigSchema } from "./config-schema.ts";
import { zalouserOnboardingAdapter } from "./onboarding.ts";
import { probeZalouser } from "./probe.ts";
import { sendMessageZalouser } from "./send.ts";
import { collectZalouserStatusIssues } from "./status-issues.ts";
import type { ZcaFriend, ZcaGroup, ZcaUserInfo } from "./types.ts";
import { checkZcaInstalled, parseJsonOutput, runZca, runZcaInteractive } from "./zca.ts";
...
export const zalouserPlugin: ChannelPlugin<ResolvedZalouserAccount> = {
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      const { monitorZalouserProvider } = await import("./monitor.ts");
