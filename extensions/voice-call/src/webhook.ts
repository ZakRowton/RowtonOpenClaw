...
import type { CoreConfig } from "./core-bridge.ts";
import type { CallManager } from "./manager.ts";
import type { MediaStreamConfig } from "./media-stream.ts";
import { MediaStreamHandler } from "./media-stream.ts";
import type { VoiceCallProvider } from "./providers/base.ts";
import { OpenAIRealtimeSTTProvider } from "./providers/stt-openai-realtime.ts";
import type { TwilioProvider } from "./providers/twilio.ts";
import type { NormalizedEvent, WebhookContext } from "./types.ts";
...
export class VoiceCallWebhookServer {
  ...
  private async handleInboundResponse(callId: string, userMessage: string): Promise<void> {
    ...
    try {
      const { generateVoiceResponse } = await import("./response-generator.ts");
