...
import { zaloMessageActions } from "./actions.ts";
import { ZaloConfigSchema } from "./config-schema.ts";
import { zaloOnboardingAdapter } from "./onboarding.ts";
import { probeZalo } from "./probe.ts";
import { resolveZaloProxyFetch } from "./proxy.ts";
import { sendMessageZalo } from "./send.ts";
import { collectZaloStatusIssues } from "./status-issues.ts";
...
export const zaloPlugin: ChannelPlugin<ResolvedZaloAccount> = {
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      const { monitorZaloProvider } = await import("./monitor.ts");
