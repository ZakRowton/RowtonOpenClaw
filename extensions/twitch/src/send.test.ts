...
import { sendMessageTwitchInternal } from "./send.ts";
import {
...
describe("send", () => {
  ...
  describe("sendMessageTwitchInternal", () => {
    it("should send a message successfully", async () => {
      const { getAccountConfig } = await import("./config.ts");
      const { getClientManager } = await import("./client-manager-registry.ts");
      const { stripMarkdownForTwitch } = await import("./utils/markdown.ts");
    ...
    it("should return error when account not configured", async () => {
      const { getAccountConfig } = await import("./config.ts");
      const { isAccountConfigured } = await import("./utils/twitch.ts");
