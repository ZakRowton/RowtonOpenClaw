...
import { TwitchClientManager } from "./twitch-client.ts";
import type { ChannelLogSink, TwitchAccountConfig, TwitchChatMessage } from "./types.ts";
...
describe("TwitchClientManager", () => {
  ...
  beforeEach(async () => {
    ...
    const { resolveTwitchToken } = await import("./token.ts");
