...
import type { ResolvedBlueBubblesAccount } from "./accounts.ts";
import { fetchBlueBubblesHistory } from "./history.ts";
import {
...
import { setBlueBubblesRuntime } from "./runtime.ts";
...
vi.mock("./reactions.js", async () => {
  const actual = await vi.importActual<typeof import("./reactions.ts")>("./reactions.js");
  return {
    ...actual,
...
describe("BlueBubbles webhook monitor", () => {
  ...
  describe("webhook parsing + auth handling", () => {
    ...
    it("parses chatId when provided as a string (webhook variant)", async () => {
      const { resolveChatGuidForTarget } = await import("./send.ts");
    ...
    it("extracts chatGuid from nested chat object fields (webhook variant)", async () => {
      const { sendMessageBlueBubbles, resolveChatGuidForTarget } = await import("./send.ts");
  ...
  describe("DM pairing behavior vs allowFrom", () => {
    ...
    it("does not resend pairing reply when request already exists", async () => {
      ...
      const { sendMessageBlueBubbles } = await import("./send.ts");
  ...
  describe("ack reactions", () => {
    it("sends ack reaction when configured", async () => {
      const { sendBlueBubblesReaction } = await import("./reactions.ts");
  ...
  describe("typing/read receipt toggles", () => {
    it("marks chat as read when sendReadReceipts=true (default)", async () => {
      const { markBlueBubblesChatRead } = await import("./chat.ts");
    ...
    it("sends typing indicator when processing message", async () => {
      const { sendBlueBubblesTyping } = await import("./chat.ts");
